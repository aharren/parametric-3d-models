...
const dimensions = (options, obj) => {
  ...
  const constructMarkers = (size, position, mirror) => {
    const m = booleans.union(lengthMarkerLine({}, size), lengthMarkerText({ position }, size));
    return mirror ? transforms.mirrorX(m) : m;
  }

  const objects = [];
  if (modes[0] !== 'none' && dim[0] > 0) {
    const markers = constructMarkers(dim[0], 'bottom', mirror[0]);
    switch (modes[0]) {
      case 'top':
        objects.push(transforms.align({ grouped: true, modes: ['center', 'max', 'max'], relativeTo: [(bounds[0][0] + bounds[1][0]) / 2, bounds[0][1] - distance, bounds[1][2]] }, markers));
        break;
      case 'bottom':
      case 'default':
      default:
        objects.push(transforms.align({ grouped: true, modes: ['center', 'max', 'max'], relativeTo: [(bounds[0][0] + bounds[1][0]) / 2, bounds[0][1] - distance, bounds[0][2]] }, markers));
        break;
    }
  }
  if (modes[1] !== 'none' && dim[1] > 0) {
    const markers = constructMarkers(dim[1], (modes[1] === 'right' ? 'top' : 'bottom'), mirror[1]);
    switch (modes[1]) {
      case 'right':
        objects.push(transforms.align({ grouped: true, modes: ['min', 'center', 'max'], relativeTo: [bounds[1][0] + distance, (bounds[0][1] + bounds[1][1]) / 2, bounds[0][2]] }, transforms.rotateZ(utils.degToRad(-90), markers)));
        break;
      case 'left':
      case 'default':
      default:
        objects.push(transforms.align({ grouped: true, modes: ['max', 'center', 'max'], relativeTo: [bounds[0][0] - distance, (bounds[0][1] + bounds[1][1]) / 2, bounds[0][2]] }, transforms.rotateZ(utils.degToRad(-90), markers)));
        break;
    }
  }
  if (modes[2] !== 'none' && dim[2] > 0) {
    const markers = constructMarkers(dim[2], 'bottom', mirror[2]);
    switch (modes[2]) {
      case 'right':
        objects.push(transforms.align({ grouped: true, modes: ['min', 'max', 'center'], relativeTo: [bounds[1][0] + distance, bounds[0][1] - distance, (bounds[0][2] + bounds[1][2]) / 2] }, transforms.rotate([0, utils.degToRad(90), utils.degToRad(45)], markers)));
        break;
      case 'left':
      case 'default':
      default:
        objects.push(transforms.align({ grouped: true, modes: ['max', 'max', 'center'], relativeTo: [bounds[0][0] - distance, bounds[0][1] - distance, (bounds[0][2] + bounds[1][2]) / 2] }, transforms.rotate([0, utils.degToRad(90), utils.degToRad(-45)], markers)));
        break;
    }
  }
...
const main = (params) => {
  ...
  objects[numObjects + 3] = dimensions({ modes: ['bottom', 'right', 'right'], dimensions: [11, 12, 13], distance: 5 }, objects[3]);
