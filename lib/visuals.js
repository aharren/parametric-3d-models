...
const dimensions = (options, obj) => {
  const modes = arrays.to3D(options.modes ?? ['default']);
...
const main = (params) => {
  ...
  objects.push(dimensions({}, objects[0]));
  objects.push(dimensions({ modes: ['default', 'none'] }, objects[1]));
  objects.push(dimensions({ modes: ['default', 'none', 'default'] }, objects[2]));
