...
const tubeElliptic = (options) => {
  ...
  const startOuterRadius = arrays.to2D(options.startOuterRadius ?? [10, 10]);
  const startInnerRadius = arrays.to2D(options.startInnerRadius ?? [8, 8]);
  const endOuterRadius = arrays.to2D(options.endOuterRadius ?? startOuterRadius);
  const endInnerRadius = arrays.to2D(options.endInnerRadius ?? startInnerRadius);
