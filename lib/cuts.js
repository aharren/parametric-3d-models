...
// apply a V cut at the top of the given objects, with the given x/y widths/depths
const vCutTop = (options, ...objects) => {
  const widths = options.widths ?? [10, 0];
  const depths = options.depths ?? [10, 0];
  ...
  const cut = translate([boundingBox[1][0] - dimensions[0] / 2, boundingBox[1][1] - dimensions[1] / 2, boundingBox[1][2]],
    !options.widths[0] ? [] : align({ modes: ['center', 'center', 'max'] },
      rotate([-Math.PI / 2, 0, Math.PI / 2],
        align({ modes: ['min', 'min', 'center'] },
          prism(widths[0], depths[0], dimensions[1])
        )
      ),
    ),
    !options.widths[1] ? [] : align({ modes: ['center', 'center', 'max'] },
      rotate([-Math.PI / 2, 0, 0],
        align({ modes: ['min', 'min', 'center'] },
          prism(widths[1], depths[1], dimensions[1])
        )
      )
    )
  );
...
const main = (params) => {
  ...
  // vCutTop
  objects.push(colorize.red(vCutTop({ widths: [40, 0], depths: [10, 0] }, cube({ size: 40 }))));
  objects.push(colorize.green(vCutTop({ widths: [0, 10], depths: [0, 10] }, cube({ size: 40 }))));
  objects.push(colorize.blue(vCutTop({ widths: [20, 20], depths: [10, 10] }, cube({ size: 40 }))));
